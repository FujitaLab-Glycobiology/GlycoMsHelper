...
Construct the glycan library based on user-defined glycan type
(different glycan types have different rules), monosaccharides, and
adducts.
...
- **`glycan_type`** (`character`) The type of glycan library to
  generate. Options include: `N_glycan`, `O_glycan`, `GSL`, `GPI`, or a
  custom user-defined string.

- **`min_charge_state`** (`numeric`) The minimum charge state of the
  glycans in the library, default: 1

- **`max_charge_state`** (`numeric`) The maximum charge state of the
  glycans in the library, default: 3

- **`monosaccharides_additional_customized_list`**
  (`named numeric vector`) This option is used when the user wants to
  include monosaccharides that are NOT one of Hex, HexNAc, dHex, Neu5Ac,
  HexA, Neu5Gc, Pentose, and KDN.  
  Warning: the name of the customized monosaccharides should be
  different fromom Hex, HexNAc, dHex, Neu5Ac, HexA, Neu5Gc, Pentose,
  KDN, if the customized monosaccharides name are same as Hex, HexNAc,
  dHex, Neu5Ac, HexA, Neu5Gc, Pentose, KDN, the scripts will check to
  make sure that the molecular weight difference is smaller than 0.01,
...
- **`min_num_monosaccharides_additional_customized_list`** and
  **`max_num_monosaccharides_additional_customized_list`**
  (`named numeric vector`) Define the minimum and maximum number for the
  customized monosaccharides in the library, the name of the vector
  should be same as `monosaccharides_additional_customized_list`, and
  the value should be numeric. **Example:**
...
- **`adduct_type`** (`character`) Built-in adducts: H, Na, K, Li, Mg.  
  For non-built-in adducts, leave this empty and use the customized
  arguments below (`adduct_monoisotopic_customized_list` and
  `adduct_monoisotopic_customized_charge_state_list`)
...
- **`adduct_monoisotopic_customized_list`** (`named numeric vector`)
  Defines non-built-in adduct names and their corresponding monoisotopic
  weights. Names must different from built-in adducts.  
...
- **`adduct_monoisotopic_customized_charge_state_list`**
  (`named numeric vector`) Define the customized adducts charge state.
  The name of the vector should be same as
  `adduct_monoisotopic_customized_list`, and the value should be numeric
...
- **`derivatization_type`** and
  **`derivatization_monoisotopic_weight_customized`**
  ‘derivatization_type’ could be ProA, AB, PA or customized
  derivatization type.  
  If setting it to “customized”, you must provide the companion
  `derivatization_monoisotopic_weight_customized` value.
...
- **`min_Hex_num`** `numeric` Defalut: 0

- **`max_Hex_num`** `numeric` Defalut: 12

- **`min_HexNAc_num`** `numeric` Defalut: 2

- **`max_HexNAc_num`** `numeric` Defalut: 10

- **`min_dHex_num`** `numeric` Defalut: 0

- **`max_dHex_num`** `numeric` Defalut: 4

- **`min_Neu5Ac_num`** `numeric` Defalut: 0

- **`max_Neu5Ac_num`** `numeric` Defalut: 4

- **`min_HexA_num`** `numeric` Defalut: 0

- **`max_HexA_num`** `numeric` Defalut: 2

- **`min_Neu5Gc_num`** `numeric` Defalut: 0

- **`max_Neu5Gc_num`** `numeric` Defalut: 4

- **`min_Pentose_num`** `numeric` Defalut: 0

- **`max_Pentose_num`** `numeric` Defalut: 0

- **`min_KDN_num`** `numeric` Defalut: 0

- **`max_KDN_num`** `numeric` Defalut: 0

- **`min_total_monosaccharides_num`** `numeric` Defalut: 2
...
1.  Extraction: For each candidate composition mapped in STEP 7, the
    function extracts the MS1 isolation windows defined by
    `ms1_window_left` and `ms1_window_right` around the precursor m/z of
    each likely glycan MS2 spectrum.

2.  Simulation: The theoretical isotopic distribution of each candidate
    composition is simulated based on its structural molecular formula.

3.  Scoring: A cosine similarity score is calculated between the binned
    experimental and theoretical isotopic envelopes.

4.  Selection: The composition with the highest cosine similarity score
    is selected as the top assignment for that specific MS2 spectrum.
...
- **`spectrum_matching_info`** (`dataframe`) The candidate matches for
  each MS2 spectrum, typically retrieved from STEP7

- **`molecular_names`** (`character`) Names of the monosaccharides,
  adducts, and corresponding label in the glycan library
...
- **`molecular_formula_list`** (`named character vector`) Molecular
  formulas for every entity declared in `molecular_names`. Vector names
  must perfectly match `molecular_names`.  
  Note: Elements with a single atom count must explicitly state 1 (e.g.,
  `ProA = 'C13H23N3O1'` instead of `'C13H23N3O'`) to ensure proper
  parsing downstream.
...
- **`ms_data`** (`MS data`) The raw or filtered mass spectrometry data
  prior to any destructive spectrum-denoising steps.

- **`ms1_window_left`** (`numeric`) Isolation window left boundary for
  extracting the experimental MS1 isotopic distribution of each likely
  glycan MS2 spectrum.

- **`ms1_window_right`** (`numeric`) Isolation window right boundary for
  extracting the experimental MS1 isotopic distribution of each likely
  glycan MS2 spectrum.

- **`bin_width`** (`numeric`) Bin width used to calculate the cosine
