...
- **`spectrum_matching_info`** (`dataframe`) The matching info of each
  MS2 spectrum, typically retrieved from STEP7
...
- **`molecular_formula_list`** (`named character vector`) Molucular
  formulas for each monosaccharide, adduct, and label in the glycan
  library. The names of the vector should match the `molecular_names`
  parameter. For some molecular with only one molecular, like the
  `ProA = 'C13H23N3O1'`, theere is only one Oxygen atom, so the
  molecular formula is ‘C13H23N3O1’ not ‘C13H23N3O’.
...
- **`ms_data`** (`MS data`) The MS data before denoising.

- **`ms1_window_left`** (`numeric`) Isolation window left boundary for
  extracting the MS1 isotopic distribution of each likely glycan MS2
  spectrum.
...
- **`threshold_iso_probalility`** (`numeric`) The minimum relative
  ...
  the comparison. See `threshold` parameters in `enviPat::isopattern()`
  for more details.
